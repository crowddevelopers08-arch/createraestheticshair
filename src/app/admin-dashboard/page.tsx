...
export default function LeadsTable({
  ...
  const sortedLeads = [...leads].sort((a, b) => {
    ...
    const rawA = a[key as keyof Lead]
    const rawB = b[key as keyof Lead]

    let aVal: string | number
    let bVal: string | number

    if (key === "createdAt" || key === "updatedAt" || key === "syncedAt") {
      aVal = new Date(rawA as string).getTime()
      bVal = new Date(rawB as string).getTime()
    } else if (typeof rawA === "string") {
      aVal = rawA.toLowerCase()
      bVal = ((rawB as string) ?? "").toLowerCase()
    } else {
      aVal = String(rawA ?? "")
      bVal = String(rawB ?? "")
    }
  ...
  const filteredLeads = sortedLeads.filter((lead) => {
    const matchesSearch =
      ...
      safeString(lead.concern).includes(safeString(searchTerm)) ||    // Creator Aesthetic Clinic specific
  ...
  const exportToCSV = () => {
    ...
    a.href = url
    a.download = `creator-aesthetic-leads-${new Date().toISOString().split("T")[0]}.csv`
    a.click()
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle className="text-2xl font-bold text-gray-900">
                Creator Aesthetic Clinic — Leads
              </CardTitle>
