...
export default function TreatThankYouPage() {
  return (
    <>
      <main className="min-h-screen bg-[#FFFFFF]">
        ...
        <div className="px-4 py-16 max-[470px]:py-6 flex flex-col items-center justify-center">
          <div className="w-full max-w-lg bg-white rounded-3xl shadow-[0_8px_40px_rgba(92,61,143,0.12)] overflow-hidden">
            {/* Header */}
            <div className="bg-[#5C3D8F] px-8 py-8 flex flex-col items-center text-center">
              <div className="w-16 h-16 rounded-full bg-white/15 border-2 border-[#D4607A] flex items-center justify-center mb-4">
            ...
            <div className="px-8 py-8">
              <p className="text-[#4A4A4A] text-sm leading-relaxed text-center mb-7">
                Dr. Sai&apos;s team will call you within{" "}
                <span className="font-semibold text-[#5C3D8F]">24 hours</span> to
                schedule your consultation. We look forward to helping you.
              </p>

              {/* Steps */}
              <div className="bg-[#F8F4FF] rounded-2xl p-5 mb-7 space-y-4">
                <p className="text-xs font-semibold text-[#5C3D8F] uppercase tracking-wider">
                  What happens next
                </p>
                {[
                  ...
                  <div key={step} className="flex items-start gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#5C3D8F] flex items-center justify-center text-white text-xs font-bold shrink-0 mt-0.5">
                      {step}
                    </span>
                    <p className="text-[#1C1C1C] text-sm">{text}</p>
                  </div>
                ))}
              </div>
          ...
          <p className="text-[#787878] text-xs mt-6 text-center">
